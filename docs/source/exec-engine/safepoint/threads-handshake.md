...
:::{figure-md} 图: Threads Handshake

<img src="threads-handshake.drawio.svg" alt="图: Threads Handshake">

*图: Threads Handshake*
:::
*[用 Draw.io 打开](https://app.diagrams.net/?ui=sketch#Uhttps%3A%2F%2Fjvm-insider.mygraphql.com%2Fzh-cn%2Flatest%2F_images%2Fthreads-handshake.drawio.svg)*



发起 Handshake 的线程叫 `handshaker`。它创建 `HandshakeOperation` 对象。
一个 `HandshakeOperation` 对象 对象可以对应多个目标线程，叫 `handshakee` 。只有所有 `handshakee` 的 `do_thread` 都运行过了，`HandshakeOperation` 才算完成。
...
1. Handshakee 线程 Polling Safepoint
2. Handshaker 发起 Handshake。有两种方式：
   - `Handshake::execute(...)`
   - `VM_HandshakeAllThreads` 是一个 `VM_Operation` ，过程经由 VM Thread 发起 global safepoint
3. Arming Handshakee
4. Handshakee check safepoint，执行 handshake closure
5. Handshaker 结束
