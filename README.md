...
本书不是一本使用手册。更不是从使用者角度，教如何深入浅出学习 JVM。不会布道 JVM 有如何如何强大之功能，更不会教如何使用 JVM。这方面网上已经有太多非常优秀的书、文章、文档了。

> 🤷 : [Yet, another](https://en.wikipedia.org/wiki/Yet_another) JVM User Guide?  
> 🙅 : No!
...
书里说的，只是在我研究与使用了 JVM 一段时间后，的思考与记录。我不是专家，更不是 JVM Committer。连互联网大厂员工也不是。只是排查过一些 JVM/Envoy 相关的功能与性能问题，浏览和 Debug 过一些 JVM/Envoy 的代码。

在研究 JVM 过程中。发现网上是有很多非常有价值的资讯。但是，要么主要是从使用者出发，没说实现机理；要么就是说了机理，也说得很好，但内容少了系统化和连贯性。

### 读者对象
本书主要讲 JVM/Envoy 的设计、实现机制。假设读者已经有一定的 JVM 使用经验。
